...
export function AudioDetailView({ audioId }: AudioDetailViewProps) {
    ...
    try {
      ...
      if (audioResponse.ok) {
        ...
        if (audioData.status === 'completed') {
          const transcriptResponse = await fetch(`/api/v1/transcription/${audioId}/transcript`, {
            headers: {
              'X-API-Key': 'dev-api-key-123',
            },
          })
  ...
  const initializeWaveSurfer = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error('Audio file request failed:', response.status, response.statusText)
          ...
          <div className="bg-white dark:bg-gray-700 rounded-xl p-6">
            <div className="text-center">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-50 mb-2">
                {audioFile.status === 'processing' && 'Transcription in Progress'}
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {audioFile.status === 'processing' && 'Please wait while we process your audio file...'}
                {audioFile.status === 'pending' && 'Your audio file is in the transcription queue.'}
                {audioFile.status === 'uploaded' && 'Start transcription from the audio files list.'}
                {audioFile.status === 'failed' && 'There was an error processing your audio file.'}
              </p>
            </div>
          </div>
        )}
