<script lang="ts">
	...
	onMount(async () => {
		...
		if (file?.id) {
			...
			// Set up polling to refresh file data while viewing a transcription
			// This ensures we get real-time status updates
			refreshInterval = setInterval(async () => {
				if (file?.id && isOpen) {
					// Refresh more frequently if still processing
					if (file.transcriptionStatus !== 'completed' && file.transcriptionStatus !== 'failed') {
						...
						// Refresh less frequently if completed (just to catch any changes)
						if (Math.random() < 0.2) { // ~20% chance to refresh completed files
							await audioFiles.refresh();
						}
					}
				}
	...
	// Function to generate title from transcript using LLM
	async function generateTitle() {
		...
		try {
			...
			// Create a suitable prompt for title generation
			const titlePrompt = "Generate a concise, descriptive title for this transcript that captures its main subject or theme. The title should be 3-8 words. The response should ONLY be the title, without any additional text, quotes, or explanation.";
			...
			const response = await apiFetch('/api/summarize', {
				...
				body: JSON.stringify({
					...
					processThinking: true, // We want a clean title without thinking sections
					maxLength: 50 // Keep title reasonably short
				})
			});
			...
			// Get the title and remove quotes if present
			...
			// Also update the local title state
