...
func (ps *ParakeetService) ProcessJobWithProcess(ctx context.Context, jobID string, registerProcess func(*exec.Cmd)) error {
	...
	if job.Parameters.Diarize {
		...
		if canRunDiarization {
			fmt.Printf("DEBUG: Running diarization for Parakeet job %s with model: %s\n", jobID, job.Parameters.DiarizeModel)
...
func (ps *ParakeetService) runDiarization(audioPath string, transcriptPath string, params models.WhisperXParams) error {
	...
	if params.DiarizeModel == "nvidia_sortformer" {
		fmt.Printf("DEBUG: Using NVIDIA Sortformer diarization for Parakeet job\n")
		...
		fmt.Printf("DEBUG: Using Pyannote diarization for Parakeet job\n")
